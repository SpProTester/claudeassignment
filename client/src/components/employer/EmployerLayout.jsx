...
function SidebarNav({ user, onLogout }) {
  const { data } = useQuery({
    queryKey: ['employer', 'app-stats-badge'],
    queryFn: () => employerService.listAllApplications({ limit: 1, atsStage: 'applied' }),
    staleTime: 60_000,
    retry: false,
  });
  const newAppCount = data?.data?.stats?.applied ?? 0;
  ...
  return (
    <nav className="flex flex-col h-full bg-white">
      ...
      <div className="flex-1 px-3 py-4 overflow-y-auto space-y-0.5">
        ...
        {/* Job Applications — with new-applications badge */}
        <NavLink to="/employer/applications"
          ...
          <span className="flex-1">Job Applications</span>
          {newAppCount > 0 && (
            <span className="ml-auto flex items-center justify-center min-w-[20px] h-5 px-1.5 rounded-full bg-primary-600 text-white text-[10px] font-bold">
