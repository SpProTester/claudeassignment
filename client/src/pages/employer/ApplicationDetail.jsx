...
import ScheduleInterviewModal from '../../components/employer/ScheduleInterviewModal.jsx';
...
const MEETING_PROVIDER_META = {
  microsoft_teams: { label: 'Microsoft Teams', color: 'bg-indigo-100 text-indigo-700' },
  google_meet:     { label: 'Google Meet',     color: 'bg-emerald-100 text-emerald-700' },
  other:           { label: 'Video Call',      color: 'bg-gray-100 text-gray-700' },
};
...
export default function ApplicationDetail() {
  ...
  // Interview scheduling mutation — sets stage to "interview" + sends meeting details
  const scheduleMutation = useMutation({
    mutationFn: (values) => employerService.scheduleInterview(id, values),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['employer', 'application', id] });
      queryClient.invalidateQueries({ queryKey: ['employer', 'all-applications'] });
      queryClient.invalidateQueries({ queryKey: ['employer', 'app-stats-badge'] });
  ...
  // Either stage button can target "interview" — open the scheduling modal instead
  // of flipping the stage directly so the employer can attach meeting details.
    ...
    <div className="space-y-6 pb-10">
        ...
        <div className="flex items-start gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {ACTION_BUTTONS.filter((a) => (
              a.stage === 'interview' ? !app.interviewMeetingLink : a.stage !== app.atsStage
            )).map((action) => (
              <button
                key={action.stage}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-5">
          ...
          {/* Interview details */}
          {app.atsStage === 'interview' && app.interviewMeetingLink && (
            <div className="bg-white rounded-2xl border border-orange-200 p-5">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-sm font-bold text-gray-900">Interview Scheduled</h3>
                <button
              ...
              <div className="space-y-2.5 mb-4">
                <div className="flex items-start gap-3">
                  <span className="text-xs text-gray-400 font-medium w-16 shrink-0 pt-0.5">When</span>
                  <span className="text-sm text-gray-800 font-semibold">{formatMeetingTime(app.interviewScheduledAt)}</span>
                </div>
                <div className="flex items-start gap-3">
                  <span className="text-xs text-gray-400 font-medium w-16 shrink-0 pt-0.5">Where</span>
                  <span className={`inline-flex items-center text-xs font-bold px-2.5 py-1 rounded-full ${MEETING_PROVIDER_META[app.interviewMeetingProvider]?.color ?? 'bg-gray-100 text-gray-600'}`}>
                    {MEETING_PROVIDER_META[app.interviewMeetingProvider]?.label ?? app.interviewMeetingProvider}
                  </span>
                </div>
                {app.interviewNotes && (
                  <div className="flex items-start gap-3">
                    <span className="text-xs text-gray-400 font-medium w-16 shrink-0 pt-0.5">Note</span>
                    <span className="text-sm text-gray-600 whitespace-pre-wrap">{app.interviewNotes}</span>
                  </div>
                )}
              </div>
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center justify-center gap-1.5 w-full text-xs font-bold text-white bg-orange-500 hover:bg-orange-600 px-4 py-2.5 rounded-xl transition-colors"
              >
                Join Meeting
                <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M13.5 6H5.25A2.25 2.25 0 003 8.25v10.5A2.25 2.25 0 005.25 21h10.5A2.25 2.25 0 0018 18.75V10.5m-10.5 6L21 3m0 0h-5.25M21 3v5.25" />
      ...
      <ScheduleInterviewModal
        ...
        onSkip={() => {
          setShowScheduleModal(false);
          stageMutation.mutate('interview');
        }}
      />
    </div>
  );
