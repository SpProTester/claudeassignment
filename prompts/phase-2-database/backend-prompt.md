# Prompt 02 â€” Database Setup & Connection
...
3. DATABASE SYNC (server/server.js):
   ...
   - In development: use sequelize.sync({ alter: true }) â€” updates tables to match models
...
4. HEALTH CHECK ENDPOINT (server/src/routes/health.js):
   - GET /health â€” returns server status, uptime, environment, database status
...
- **Connection Pooling** â€” why `max: 10` matters; avoids creating a new DB connection per request
- **Sequelize authenticate() vs sync()** â€” `authenticate()` just tests the connection; `sync()` creates/alters tables; never use `sync` in production
- **Migrations vs sync** â€” migrations give you version-controlled schema changes; sync is for development convenience only
- **Environment validation** â€” fail fast on startup if config is missing; better than cryptic errors at runtime
- **Winston transports** â€” sending logs to multiple destinations simultaneously
...
## Next Step
Move to **Prompt 03 â€” Database Schema & Models** once database connects successfully.
