...
1. server/src/services/adminService.js:
   ...
   - Return { accessToken, user } — frontend uses this token for impersonation session
...
2. Auto-audit logging — create middleware:
...
3. server/src/routes/admin.js:
   ...
   POST /users/:id/impersonate    → impersonateUser (super_admin only)
   GET  /jobs/flagged             → getFlaggedJobs
   PUT  /jobs/:id/moderate        → moderateJob
   GET  /stats                    → getPlatformStats
   GET  /stats/revenue            → getRevenueStats
   POST /payments/:id/refund      → processRefund
   GET  /audit-log                → getAuditLog (super_admin only)
   GET  /categories               → CRUD for job categories
...
4. Flag system for jobs:
   ...
   - Public route: POST /jobs/:id/flag → { reason } — any logged-in user can flag
...
- **Impersonation** — legitimate admin debugging tool; MUST be logged in audit trail with `impersonated_by`; token should be short-lived; ideally shown as banner in UI
- **Audit logs** — immutable record of all admin actions; use INSERT only, never UPDATE or DELETE audit records; keep separate from regular logs
- **Aggregation queries** — platform stats require careful query design; consider caching stats that are expensive to compute
- **Stripe refunds** — `stripe.refunds.create` requires the `payment_intent` ID; refunds can be partial or full; always update your DB after Stripe confirms
...
- [ ] Flag count ≥ 3 auto-moves job to flagged queue
