# Prompt 03 — Database Schema & Sequelize Models
...
1. User (server/src/models/User.js)
   ...
   Hooks: beforeCreate — hash password with bcrypt(12) if password provided
   Instance method: validatePassword(plainText) — bcrypt.compare
   Instance method: toSafeJSON() — return user without password_hash, mfa_secret

2. SeekerProfile (server/src/models/SeekerProfile.js)
   Fields: id(UUID PK), user_id(UUID FK→User), headline(STRING 220 null),
   ...
   profile_visibility(ENUM: public,employers_only,private — default public),
...
3. EmployerProfile (server/src/models/EmployerProfile.js)
   Fields: id(UUID PK), user_id(UUID FK→User), company_name(STRING 255),
   ...
   subscription_plan(ENUM: free,professional,business,enterprise — default free),
   subscription_expires_at(DATE null), job_post_limit(INTEGER 2),
   stripe_customer_id(STRING null)

4. JobListing (server/src/models/JobListing.js)
   Fields: id(UUID PK), employer_id(UUID FK→EmployerProfile),
   category_id(UUID FK→JobCategory null), title(STRING 255), slug(STRING 300, unique),
...
5. Application (server/src/models/Application.js)
   Fields: id(UUID PK), job_id(UUID FK→JobListing), seeker_id(UUID FK→SeekerProfile),
   resume_id(UUID FK→Resume null), cover_letter(TEXT null),
   ats_stage(ENUM: applied,reviewing,shortlisted,interview,offer,hired,rejected — default applied),
...
6. Resume (server/src/models/Resume.js)
   Fields: id(UUID PK), seeker_id(UUID FK→SeekerProfile), file_name(STRING 255),
   storage_path(TEXT), file_size_bytes(INTEGER), mime_type(STRING 100),
   is_default(BOOLEAN false), label(STRING 100 null),
   parsed_data(JSONB null), parse_status(ENUM: pending,processing,done,failed — default pending),
   parse_confidence(DECIMAL(5,2) null)

7. WorkExperience (server/src/models/WorkExperience.js)
   Fields: id(UUID PK), seeker_id(UUID FK→SeekerProfile), job_title(STRING 255),
...
8. Education (server/src/models/Education.js)
   Fields: id(UUID PK), seeker_id(UUID FK→SeekerProfile), degree(STRING 255),
...
10. SeekerSkill (server/src/models/SeekerSkill.js) — junction table
    Fields: seeker_id(UUID FK), skill_id(UUID FK), 
    proficiency(ENUM: beginner,intermediate,advanced,expert),
    years_experience(INTEGER null)

11. JobSkill (server/src/models/JobSkill.js) — junction table
...
12. JobCategory (server/src/models/JobCategory.js)
    ...
    icon(STRING 50 null), parent_id(UUID FK→JobCategory null), job_count(INTEGER 0)

13. SavedJob (server/src/models/SavedJob.js)
    Fields: id(UUID PK), seeker_id(UUID FK→SeekerProfile), job_id(UUID FK→JobListing)
    Unique: [seeker_id, job_id]

14. JobAlert (server/src/models/JobAlert.js)
    Fields: id(UUID PK), seeker_id(UUID FK→SeekerProfile), name(STRING 100),
    ...
    frequency(ENUM: daily,weekly — default daily), is_active(BOOLEAN true),
    last_sent_at(DATE null)

15. Notification (server/src/models/Notification.js)
    Fields: id(UUID PK), user_id(UUID FK→User), type(STRING 100),
...
16. AuditLog (server/src/models/AuditLog.js)
    Fields: id(UUID PK), user_id(UUID FK→User null), action(STRING 100),
...
```
server/src/models/
├── index.js              ← associations hub + exports
├── User.js
├── SeekerProfile.js
├── EmployerProfile.js
├── JobListing.js
...
- **UUID vs Integer PKs** — UUIDs prevent sequential ID enumeration attacks; use `DataTypes.UUID` + `defaultValue: DataTypes.UUIDV4`
- **Sequelize associations** — `hasOne`, `hasMany`, `belongsTo`, `belongsToMany`; the difference between FK placement in each
- **JSONB in PostgreSQL** — stores JSON with indexing capability; used for `parsed_data` and `metadata`
- **Sequelize hooks** — `beforeCreate`, `beforeUpdate` for password hashing; `afterCreate` for profile initialization
- **Junction tables** — `belongsToMany` through a model; allows extra attributes on the join (like `proficiency`)
- **Unique constraints** — `unique: true` on single field vs `{ fields: ['job_id', 'seeker_id'] }` for composite
...
## Next Step
Move to **Prompt 04 — Authentication Backend** once all models sync without errors.
