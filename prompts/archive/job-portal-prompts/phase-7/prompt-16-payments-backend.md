# Prompt 16 — Payment & Subscriptions Backend
...
4. server/src/routes/payments.js:
   POST /checkout         → authenticateToken + authorizeRole('employer'), createCheckoutSession
   POST /webhook          → express.raw({ type: 'application/json' }), handleWebhook (NO auth middleware)
   POST /cancel           → authenticateToken + authorizeRole('employer'), cancelSubscription
   GET  /invoices         → authenticateToken + authorizeRole('employer'), getInvoices
   GET  /portal           → authenticateToken + authorizeRole('employer'), getBillingPortalSession
   GET  /plans            → getAllPlans (public)

   CRITICAL: Webhook route MUST use express.raw() not express.json() — Stripe needs raw body for signature verification
...
- **Stripe Checkout** — hosted payment page by Stripe; no card data touches your server; PCI compliant by default
- **Stripe webhooks** — Stripe calls your server when events happen (payment succeeded, failed, subscription cancelled); you MUST handle these asynchronously
- **Webhook signature verification** — `stripe.webhooks.constructEvent(rawBody, sig, secret)` prevents fake webhook calls; MUST use raw body (not parsed JSON)
- **`cancel_at_period_end`** — subscription stays active until period ends, then cancels; better UX than immediate cancellation
- **Stripe billing portal** — hosted page where customers manage payment methods, view invoices, cancel; saves you building all that UI
...
- [ ] Complete checkout → employer plan updated in DB
- [ ] Failed payment → employer notified by email
- [ ] Subscription cancel → scheduled to cancel at period end
- [ ] Webhook signature verification rejects tampered payloads

---

## Next Step
**Prompt 17 — Billing Frontend**
