# Prompt 25 â€” Performance Optimization
...
1. Redis Caching with ioredis:
   ...
   - get(key) â†’ redis.get(key), JSON.parse
   - set(key, value, ttlSeconds) â†’ redis.setex(key, ttl, JSON.stringify(value))
   - del(key) â†’ redis.del(key)
   - delPattern(pattern) â†’ redis.keys(pattern) then del each
   - Use try/catch â€” if Redis is down, return null (graceful degradation)
   ...
   - cacheRoute(ttl) â€” middleware factory
...
2. Database Query Optimization:
   ...
   server/src/database/optimize.sql â€” run these commands:
   ...
   - No COUNT(*) needed â€” faster for large datasets
...
7. React Query optimization (client/src/api/queryConfig.js):
   export const queryClient = new QueryClient({
     defaultOptions: {
       queries: {
         staleTime: 60 * 1000,         // 1 minute â€” don't refetch if data < 1min old
   ...
   - Categories: staleTime: 60 * 60 * 1000 (1 hour â€” rarely changes)
   - Notifications: staleTime: 30 * 1000 (30 seconds â€” need freshness)
...
10. Prefetching critical routes:
    ...
    - On hover of "My Applications" nav link â†’ prefetch applications page
    - On job search page â†’ prefetch job detail for hovered job cards (after 300ms hover)
...
- **Cache invalidation strategy** â€” TTL-based (expire after N seconds) vs event-based (explicitly delete on update); TTL is simpler; event-based is more accurate
- **Cache key design** â€” include version in key `v1:search:...` to allow cache busting on schema changes; include user ID for personalized data
- **Partial PostgreSQL indexes** â€” `WHERE status = 'active'` on index means only active rows are indexed; much smaller and faster than full-table index
- **Cursor pagination vs offset** â€” offset-based (`OFFSET 1000`) requires DB to scan and skip 1000 rows; cursor-based uses indexed WHERE clause; order of magnitude faster for large datasets
- **React.lazy + Suspense** â€” dynamic imports create separate chunks; only downloaded when route is visited; reduces initial bundle
- **`staleTime` vs `gcTime`** â€” `staleTime`: data is "fresh" for this long, no refetch; `gcTime`: how long to keep unused data in cache before garbage collecting
