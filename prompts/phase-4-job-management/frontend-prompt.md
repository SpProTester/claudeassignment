# Prompt 11 â€” Employer Frontend Dashboard
...
2. client/src/pages/employer/DashboardPage.jsx:
   ...
   - Chart: Applications over last 30 days (Line chart â€” Recharts)
...
4. client/src/pages/employer/PostJobPage.jsx (multi-step form):
   Progress stepper at top: Step 1 â†’ Step 2 â†’ Step 3 â†’ Step 4
   ...
   Step 3 â€” Description & Requirements:
...
ALSO CREATE:
- client/src/api/employerApi.js â€” all employer API calls
- client/src/hooks/useEmployer.js â€” React Query hooks
- client/src/components/employer/KanbanBoard.jsx â€” standalone Kanban component
- client/src/components/employer/ApplicantCard.jsx â€” draggable card
- client/src/components/employer/JobForm.jsx â€” multi-step form logic
...
- **@dnd-kit** â€” modern drag-and-drop for React; `DndContext`, `useDraggable`, `useDroppable`; more performant than react-beautiful-dnd
- **Optimistic updates in React Query** â€” `onMutate` to update cache immediately, `onError` to rollback
- **Multi-step forms** â€” `useState` for current step; validate each step before allowing Next; persist to `localStorage`
- **Recharts** â€” `LineChart`, `BarChart`, `PieChart` with `ResponsiveContainer` for responsive sizing
- **PDF embed** â€” `<iframe src={resumeUrl} />` works for PDFs in most browsers; fallback to download link
