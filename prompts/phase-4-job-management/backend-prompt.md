# Prompt 10 â€” ATS (Applicant Tracking System) Backend

## Phase
Phase 4 â€” Employer Module
...
1. server/src/services/atsService.js:
   ...
   - Include: SeekerProfile â†’ User (name, email, avatar),
   ...
   - Verify ownership
   - Valid stages: appliedâ†’reviewingâ†’shortlistedâ†’interviewâ†’offerâ†’hired/rejected
   ...
   getNotes(userId, applicationId) â€” return all notes for application
...
2. server/src/routes/employer.js â€” ATS routes:
   GET    /jobs/:jobId/applicants              â†’ getApplicants
   GET    /applicants/:id                      â†’ getApplicantDetail
   PUT    /applicants/:id/stage               â†’ updateStage
   POST   /applicants/:id/notes               â†’ addNote
   GET    /applicants/:id/notes               â†’ getNotes
   PUT    /applicants/:id/rating              â†’ setRating
   POST   /applicants/:id/interview           â†’ scheduleInterview
   POST   /applicants/:id/email               â†’ emailApplicant
   GET    /jobs/:jobId/applicants/export      â†’ exportApplicants
...
Routes:
   GET /analytics/overview â†’ getOverview
   GET /analytics/jobs/:id â†’ getJobAnalytics
```
...
- **Application state machine** â€” define valid transitions; log every change in `ApplicationHistory` for audit trail
- **CSV streaming** â€” stream large CSV exports instead of buffering in memory; use `res.setHeader('Content-Disposition', 'attachment; filename=applicants.csv')`
- **Notification + email pattern** â€” always create DB notification AND send email as separate concerns; if email fails, DB notification still exists
- **Stage history** â€” store every stage change with timestamp; enables "time in stage" analytics and seeker-facing timeline display
- **Bulk operations** â€” export must handle 1000+ applicants efficiently; use Sequelize streaming or raw queries with pagination
