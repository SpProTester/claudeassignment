# Prompt 09 â€” Employer Job Posting Backend
...
1. server/src/utils/helpers.js â€” ADD:
...
2. server/src/services/jobService.js:
   ...
   - Validate status transitions:
     draft â†’ active (valid)
     active â†’ paused (valid)
     paused â†’ active (valid)
     active/paused â†’ closed (valid)
     closed â†’ any (invalid â€” can't reopen)
     expired â†’ any (invalid)
...
4. server/src/routes/employer.js â€” job routes:
   POST   /jobs                â†’ createJobRules, createJob
   GET    /jobs                â†’ getEmployerJobs (query: status, page, limit)
   GET    /jobs/:id            â†’ getJobById
   PUT    /jobs/:id            â†’ createJobRules (partial), updateJob
   DELETE /jobs/:id            â†’ deleteJob
   PUT    /jobs/:id/status     â†’ changeJobStatus
   GET    /jobs/:id/stats      â†’ getJobStats
...
6. COMPANY PROFILE ROUTES (server/src/routes/employer.js):
   ...
   PUT    /profile             â†’ updateCompanyProfile
   POST   /profile/logo        â†’ logoUpload.single('logo'), uploadLogo
...
- **Quota enforcement** â€” check at creation AND status change to active; can't bypass by creating in draft then activating
- **Status machine** â€” define which transitions are valid; `closed â†’ active` should be blocked (forces re-posting)
- **Cron jobs** â€” `node-cron` schedules functions using cron syntax; runs in same Node.js process; for production consider separate worker process
- **Slug uniqueness** â€” append random chars to handle duplicate job titles; always check DB for collision
- **Soft deletes** â€” setting `deleted_at` keeps data for analytics and audit; add `WHERE deleted_at IS NULL` to all normal queries
...
- [ ] Status transitions: draftâ†’active works, closedâ†’active returns error
...
## Next Step
**Prompt 10 â€” ATS (Applicant Tracking) Backend**
