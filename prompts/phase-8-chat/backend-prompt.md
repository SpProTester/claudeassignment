...
3. Update server/src/services/resumeService.js:
   ...
   - Add route: POST /seekers/resume/:id/parse â†’ manually trigger re-parse

4. server/src/routes/seekers.js â€” ADD:
   POST /resume/:id/parse â†’ parseResumeWithAI trigger

FRONTEND:

5. client/src/pages/seeker/ResumePage.jsx â€” UPDATE:
   ...
   Link: "See what was extracted â†’" opens modal
...
- **OpenAI structured outputs** â€” prompt the model to return only JSON; validate the structure; never trust AI output blindly
- **`gpt-4o-mini` vs `gpt-4`** â€” mini is 10x cheaper and fast enough for parsing; use gpt-4 only if accuracy requirements are very high
- **`setImmediate`** â€” defers execution to next event loop iteration; allows HTTP response to return before heavy async work starts
- **`mammoth`** â€” extracts clean text from DOCX files; better than custom XML parsing
- **Confidence score** â€” ask the model to self-assess; use it to flag low-confidence parses for manual review

---

## Validation Checklist

- [ ] Upload PDF â†’ profile auto-populated with extracted data
- [ ] Upload DOCX â†’ extracted correctly via mammoth
- [ ] Uploaded image-based PDF â†’ marked as 'failed' with helpful message
...
## Next Step
**Prompt 19 â€” AI Job Recommendations**
